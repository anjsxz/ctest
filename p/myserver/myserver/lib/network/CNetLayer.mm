...
CNetLayer::CNetLayer()
{
    ...
    char *m_sys = (char*)[str2 UTF8String];
//
    ...
    std::string value="";
    if(!GameConfig::readXml(HEADXML,BANBENHAO, value))
    {
        GameConfig::setXml(HEADXML, BANBENHAO, "1.0");
        GameConfig::readXml(HEADXML,BANBENHAO, value);
    }
    m_commfield.appver = value;
    m_commfield.snnum = snnum;//m_commfield.snnum;
    m_commfield.sysver = m_sys;
    m_commfield.verions = "1.0";
...
void CNetLayer::checkret()
{
    if(m_ret != 0)
    {
        ...
        CCNotificationCenter::sharedNotificationCenter()->postNotification(m_message[m_num - 1].c_str(), m_res[m_num - 1]);
...
void* CNetLayer::request(void *data)
{
    ...
    netlayer->m_num ++;
    if(m_cdat->DoRequest(netlayer->m_req[m],netlayer->m_res[m], 2))
    {
